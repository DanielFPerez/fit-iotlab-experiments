...
static int read(uint8_t *buf, uint8_t buf_len)
{
#define RSSI_BASE_VALUE -91
    uint8_t len;
    uint8_t rssi_reg = rf2xx_reg_read(RF2XX_DEVICE, RF2XX_REG__PHY_RSSI);
    // Check the CRC is good
    if (!(rssi_reg & RF2XX_PHY_RSSI_MASK__RX_CRC_VALID))
    {
    ...
    packetbuf_set_attr(PACKETBUF_ATTR_RSSI, (int16_t)(RSSI_BASE_VALUE + rssi));
#ifdef RF2XX_LEDS_ON
        leds_on(LEDS_GREEN);
#endif
